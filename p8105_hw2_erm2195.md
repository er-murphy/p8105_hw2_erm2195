...
- Using arguments in `read_excel`, omitting non-data entries like rows
  ...
  Data, this includes the first row, the last row, and the `Homes`
  column.
- Specifying reasonable variable names \> I think that the names
  ...
  name itself)
- Omitting rows that don’t include dumpster-specific data \> I already
  removed rows that don’t include dumpster-specific data as part of my
  import process with `read_excel`
...
- Using arguments in `read_excel`, omitting non-data entries like rows
  ...
  this includes the first row, the last row, and the `Homes` column.
- Specifying reasonable variable names \> I think that the names
  imported from the Excel document are already pretty reasonable. For
  the sake of brevity, dropping the units from weight and volume (as I
  would just document these units elsewhere, rather than in the variable
...
- Specifying the sheet in the Excel file
- Omitting non-data entries like rows with notes / figures, and columns
  containing notes using arguments in read_excel
