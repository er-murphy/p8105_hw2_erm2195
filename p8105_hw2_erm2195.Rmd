...
From the merged `538_data` data set we can see that in January from 1975 onward, in years with a democratic president, the average unemployment rate was `r filter(data_538, month == "January", year >= 1975, president == "dem") |> pull(percent_unemployment) |> mean() |> round(2)`. For comparison, the average unemployment rate over the same time period in which a republican was president was `r filter(data_538, month == "January", year >= 1975, president == "gop") |> pull(percent_unemployment) |> mean() |> round(2)`.


# Problem 2

## Cleaning `Mr. Trash Wheel` Data
First, reading in the data from the Excel file sheet `Mr. Trash Wheel`:
...
More flexible way to skip the first row, last row, and right-most column?
